...
getLightTheme() {
  return ThemeData(
      ...
      textTheme: TextTheme(button: TextStyle(color: textPrimaryColor, fontSize: 20)),
...
secondaryTextStyle({size = 16, color = textSecondaryColor, aFontFamily = fontRegular}) {
  return TextStyle(color: color, fontSize: double.parse(size.toString()).toDouble(), fontFamily: aFontFamily);
}

Widget textSecondary(text, {size = 16, color = textSecondaryColor, aFontFamily = fontRegular}) {
  return Text(text, style: secondaryTextStyle(size: size, color: color, aFontFamily: aFontFamily));
}
...
Widget text(
  ...
  return Text(
    ...
    style:
        TextStyle(fontFamily: fontFamily, fontSize: double.parse(fontSize.toString()).toDouble(), height: 1.5, color: textColor.toString().isNotEmpty ? textColor : null, letterSpacing: latterSpacing),
  );
}
